...
def startgame():
    global gamepad, game_pad , move_count
    ...
    while True:
                        ...
                        if col_rect.collidepoint(event.pos):
                            if game_pad.second_hanoi[-1] == game_pad.image_list2[a][1]:
                                if len(game_pad.mouse_traking) == 0:
                                    ...
                                    game_pad.mouse_traking.append(save_traking)
                if len(game_pad.image_list3) > 0 and len(game_pad.third_hanoi):
        ...
        if game_pad.third_hanoi == [7,6,5,4,3,2,1]:
            main.data_save.last_score = int(after_time-before_time)
            main.end_state = 3
            if int(after_time-before_time) < main.data_save.hanoi_score:
                main.data_save.hanoi_score = int(after_time-before_time)
            main.data_save.save_data()
            main.initGame(1)
        count_text = font.render("이동 횟수:{}".format(move_count),True,(0,0,0))
        gamepad.blit(count_text,(400,90))
