...
small_font = pygame.font.Font('BMJUA_ttf.ttf', 25)
...
def startgame():
    ...
    while True:
        ...
        draw_background()
        if game_count == 51:
            main.data_save.last_score = int(after_time-before_time)
            main.end_state = 2
            if int(after_time-before_time) < main.data_save.one_to_50_score or main.data_save.one_to_50_score == 0:
                main.data_save.one_to_50_score = int(after_time-before_time)
            main.data_save.save_data()
            main.initGame(1)
        game_pad.running()
        time_text = small_font.render("소요 시간: %s" % int(after_time-before_time), True, (0, 0, 0))
        count_text= small_font.render("다음 숫자: %s" % game_count,True,(0,0,0))
        gamepad.blit(count_text,(500,60))
        gamepad.blit(time_text,(500,35))
        print(int(after_time-before_time))
        # pygame.draw.rect(gamepad, (0, 0, 0), [120, 290, 30, 210], 1)
