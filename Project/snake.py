...
class Gamepad(): # 게임판
    ...
    def init_game(self):
        ...
        if self.player.positions[0] == self.item.position:
            score += 100*(random.randint(1,3))
            speed += 0.1
            game_time = game_time * 0.9
            self.player.lenup()
            self.new_item()
    ...
    while True:
        ...
        if INIT_TURN < datetime.datetime.now() - Player_move:
            try:
                ...
                main.data_save.last_score = score
                if score > main.data_save.snake_score:
                    main.data_save.snake_score = score
                main.data_save.save_data()
                main.initGame(1)
            Player_move = datetime.datetime.now()

        background()
        gamepad.blit(len_text,(SCREEN_WIDTH-120,SCREEN_HEIGHT-450))
